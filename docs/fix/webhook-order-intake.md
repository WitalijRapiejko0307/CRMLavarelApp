...
- [ ] Webhook с `"phone": "291234567"` сохраняет `375291234567`
- [ ] Webhook с `"options": "49.90"` и `"offer": "Товар А"` → `prices: [49.90]`, `sms_log: null`
- [ ] Редактирование заявки с товаром вне каталога — название видно, badge «Не на складе», ссылка на добавление товара
- [ ] Оператор может перевыбрать товар из select, если название ошибочное
- [ ] Ручное создание/редактирование заказа не сохраняется без ФИО из 3 слов
- [ ] Создание бланка Белпочты блокируется с понятной ошибкой, если ФИО неполное
- [ ] Blacklist корректно проверяет номер после нормализации
